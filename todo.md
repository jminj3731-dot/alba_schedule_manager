...
## v23 출근 시간 정각 알림 이메일
- [x] server/email.ts - 출근 시간 정각 알림 이메일 템플릿 추가 (기존 1시간 전과 별도)
- [x] server/emailScheduler.ts - 출근 시간 0~10분 전 알림 발송 로직 추가
...
## v25 구글 시트 내보내기 개선
- [x] 출퇴근 기록 탭: 실제 출퇴근 시간 없으면 예정 시간으로 자동 채우기 (빈칸 방지)
- [x] 출퇴근 기록 탭: 비고 컨럼 추가 (버튼 미입력/출근 미입력/퇴근 미입력 표시)
- [x] 스케줄 탭 가독성 개선: 타임별로 행 분리, 예정/실제 출퇴근 시간 모두 표시
- [x] 96개 테스트 통과

## v26 구글 시트 근무시간 계산 기준 변경
- [x] 출퇴근 기록 탭: 근무시간(h) 계산을 실제 출퇴근 기록 대신 스케줄 예정 시간 기준으로 변경
- [x] 스케줄 탭: 근무시간(h) 계산도 예정 시간 기준으로 변경
- [x] 급여 계산 탭: 총 근무시간 집계도 예정 시간 기준으로 변경
- [x] 97개 테스트 통과

## v27 담당자 배정 시 기본 근무 시간 자동 채우기
- [x] 원인 파악: upsertSchedule이 workerId만 저장하고 시간은 저장하지 않음
- [x] db.ts DEFAULT_TIMES 정의 (A: 17:30~22:00, B/C: 18:00~22:00)
- [x] upsertSchedule 업데이트: 담당자 배정 시 시간 비어있으면 기본값 자동 채우기
- [x] upsertSchedule 신규: 담당자 있으면 기본 시간 자동 저장
- [x] 담당자 제거 시 시간도 null로 초기화
- [x] 기존에 수동 지정된 시간은 덮어쓰지 않음
- [x] 104개 테스트 통과

## v28 매일 자정 구글 시트 자동 동기화
- [x] server/googleSheetsScheduler.ts - 매일 자정(KST 00:00) 자동 내보내기 스케줄러 구현
- [x] server/_core/index.ts에 스케줄러 시작 연동
- [x] 마지막 동기화 시간/상태 메모리 저장 및 로그 기록
- [x] routers.ts에 lastSyncInfo 조회 tRPC 라우터 추가
...
## v31 이메일 알림 타이밍 조정
- [x] 출근 시간 알림: 0~10분 전 → 0~2분 전으로 좌히기 (닥 출근 시간에 한 번)
- [x] 퇴근 시간 알림: 0~10분 전 → 8~10분 전으로 변경 (퇴근 10분 전 알림)
- [x] 127개 테스트 통과

## v32 구글 시트 내보내기 개선
- [x] googleSheets.ts: 급여 계산 탭 완전 제거
- [x] googleSheets.ts: 날짜 범위 기본값을 "KST 오늘까지 최근 3개월"로 변경 (미래 스케줄 미포함)
- [x] Settings.tsx: 급여 정산 기간 선택 UI 제거, 날짜 범위 직접 입력으로 단순화
- [x] googleSheets.test.ts: SHEETS 상수 테스트에서 급여 계산 탭 제거
- [x] 127개 테스트 통과
...
## v36 버그 수정: 이메일 요일 표시 오류 (UTC/KST 차이)
